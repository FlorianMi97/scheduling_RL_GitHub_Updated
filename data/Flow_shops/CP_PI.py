        ...
        for b in ops:
            for c in ELIGIBLE_MACHINES[a][b]:
                ...
                for j in JOBS:
                    ...
                    SETUPS[c][tmpFrom,tmpIndex] = round(dataS["setup_time"][c][jType][a] * 1000) # TODO check if it still works
...
def calculatePI(case, objective, sensitivity = "no"):
    ...
    for filestr in files:
        ...
        for s in samples:
            ...
            print(CPsolution(instance, sample, objective))
            break
            if "objective" in sample.keys():
                continue
            else:
                # sample["objective_" + str(objective)] = CPsolution(instance, sample, objective)
                sample["objective"] = CPsolution(instance, sample, objective)
